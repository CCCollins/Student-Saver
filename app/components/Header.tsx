...
import { FaCity, FaGithub, FaHeart, FaRegHeart } from "react-icons/fa";
...
const Header = ({ cities, selectedCity, setSelectedCity, isFavoritesOnly, setIsFavoritesOnly }: HeaderProps) => {
  ...
  return (
    <header className="w-full max-w-7xl mx-auto bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-xl rounded-lg p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-3">
      ...
      <div className="flex flex-col sm:flex-row w-full sm:w-auto justify-between gap-4">
        ...
        {/* Админ-кнопка, иконка GitHub и кнопка выхода */}
        <div className="flex w-full sm:w-auto justify-between gap-4 sm:mt-0">
          {isAdmin && (
          ...
          <Link href="https://github.com/CCCollins/Student-Saver" className="flex-1 sm:w-auto" target="_blank">
            <span className="w-full sm:w-12 h-12 bg-white text-gray-600 p-2 rounded-lg shadow hover:bg-gray-200 transition cursor-pointer flex items-center justify-center">
              <FaGithub size={24} />
            </span>
          </Link>
          {user && (
            <button
              onClick={handleAuth}
              className="flex items-center justify-center w-1/4 sm:w-12 h-12 bg-white text-gray-600 p-2 rounded-lg shadow hover:bg-gray-200 transition cursor-pointer"
            >
              <FiLogOut size={24} />
            </button>
          )}
